            ...
            <table id="matable" class="table table-striped" style="width:100%">
              ...
              <tbody>
                <?php 
                    ...
                    $query ="SELECT `numProfesseur`, `nomProfesseur`, `prenomProfesseur`, `suiviStage`, `sexeProfesseur` FROM `professeur_`;";  
                    $result = mysqli_query($connect, $query);  
                        //si non , affichons la liste
                        while($row = mysqli_fetch_array($result)){
                            ?>
                            <tr>
                                <td><?=$row['nomProfesseur']?></td>
                                <td><?=$row['prenomProfesseur']?></td>
                                <td><?=$row['suiviStage']?></td>
                                <td><?=$row['sexeProfesseur']?></td>

                                <!--Nous alons mettre l'id dans ce lien -->
                                <form action="" method="POST">
                                <td><a type="button" class="modifierbtn" id="<?=$row['numProfesseur']?>" data-toggle="modal" data-target="#modifierModal"><img src="img/pen.png"></a></td>
                                
                                <td><a type="button" class="suppbtn" id="<?=$row['numProfesseur']?>" data-toggle="modal" data-target="#supprimerModal"><img src="img/trash.png"></a></td>
                                </form>
                            </tr>
                            <?php                       
                    }
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form action="" method="POST">
              <div class="form-group">
                <label for="recipient-name" class="col-form-label">Nom du professeur:</label>
                <input type="text" name="nomProfesseur" class="form-control" id="recipient-name" required>
                
                <label for="recipient-name" class="col-form-label">Prénom du professeur :</label>
                <input type="text" name="prenomProfesseur" class="form-control" id="recipient-name" required>
                
                <label for="recipient-name" class="col-form-label">Suivi du stage :</label>
                <input type="text" name="Suivistage" class="form-control" id="recipient-name" required>
                
                <label for="recipient-name" class="col-form-label">Civilité :</label>
                <input type="text" name="civilite" class="form-control" id="recipient-name" required>
              </div>
          </div> 
          ...
          if(isset($_POST['btnajouter'])){
              extract($_POST);
              $Mesfonctions->createProfesseur($_POST['nomProfesseur'], $_POST['prenomProfesseur'], $_POST['Suivistage'], $_POST['civilite']);
        ...
        <div class="modal-content">
          ...
          <form action="" method="POST">
              <div class="form-group">
                ...
                <label for="recipient-name" class="col-form-label">Suivi du stage :</label>
                <input type="text" name="suivi_modif" class="form-control" id="suivi_modif" required>
                
                <label for="recipient-name" class="col-form-label">Civilité :</label>
                <input type="text" name="civil_modif" class="form-control" id="civil_modif" required>
              </div>  
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">Annuler</button>
            ...
            <?php
              ...
              extract($_POST);
              $Mesfonctions->updateProfesseur($_POST['id_modif'],$_POST['nom_modif'], $_POST['prenom_modif'], $_POST['suivi_modif'], $_POST['civil_modif']);
              echo "<meta http-equiv='refresh' content='0'>";
                } ?>
              </form>
          </div>
    ...
    <div class="modal fade" id="supprimerModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <form action="" method="POST">
              <div class="form-group">
              <label for="recipient-name" class="col-form-label">Voulez-vous vraiment supprimer ce professeur ?</label>
          ...
          <div class="modal-footer">
            ...
            <?php
              ...
              extract($_POST);
              $Mesfonctions->deleteProfesseur($_POST['id_suppr']);
              echo "<meta http-equiv='refresh' content='0'>";
                } ?>
              </form>
          </div>
...
<script>
    $(".modifierbtn").click(function(){
        ...
        $.ajax({
            type: "POST",
            url: 'getemployebyprofesseur.php',
            data: { id_row : id_row },
            success: function(data)
            {                                
                data = JSON.parse(data);
                console.log(data); 
                document.getElementById("nom_modif").value = data.nomProfesseur;                   
                document.getElementById("prenom_modif").value = data.prenomProfesseur;
                document.getElementById("suivi_modif").value = data.suiviStage;
                document.getElementById("civil_modif").value = data.sexeProfesseur;
            }
        });
    });
</script>
